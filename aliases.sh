...
function _get-grafana-secret {
  ...
  secret=${2:-grafana} # Default secret name is 'grafana'
...
alias get-kiali-secret='k exec -n istio-system deployment/kiali -- cat /var/run/secrets/kubernetes.io/serviceaccount/token'
